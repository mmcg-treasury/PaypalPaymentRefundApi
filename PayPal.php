<?php
    ...
    class PayPal
    {
        ...
        function sendRefundRequest($requestString)
        {
            ...
            if(!$curlResponse)
                return array("STATUS"=>false,"ERROR_MESSAGE"=>"RefundTransaction failed".curl_error($ch)."(".curl_errno($ch).")");
            ...
            if((0 == sizeof($aryResponse)) || !array_key_exists('ACK', $aryResponse))
                return array("STATUS"=>false,"ERROR_MESSAGE"=>"Invalid HTTP Response for POST request ($reqStr) to {$this->API_Endpoint}");
        ...
        function refundAmount($aryData)
        {
            ...
            if(trim(@$aryData['refundType'])=="")
                return array("STATUS"=>false,"ERROR_MESSAGE"=>"Refund Type is Missing");
            if(trim(@$aryData['transactionID'])=="")
                return array("STATUS"=>false,"ERROR_MESSAGE"=>"Transaction ID is Missing");
            ...
            if(strcasecmp($aryData['refundType'], 'Partial') == 0)
            {
                ...
                {
                    return array("STATUS"=>false,"ERROR_MESSAGE"=>"For Partial Refund - It is essential to mention Amount");
                }
                else
                {
                ...
                if(trim(@$aryData['currencyCode'])=="")
                    return array("STATUS"=>false,"ERROR_MESSAGE"=>"Currency Code is Missing");
            }
